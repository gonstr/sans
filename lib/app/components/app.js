...
export default class App extends Component {
  ...
  componentDidMount = () => {
    ...
    ipcRenderer.on('Show Previous Shell', () => {
      const { active, sessions } = this.state
      const index = sessions.indexOf(active)
    ...
    ipcRenderer.on('Show Next Shell', () => {
      const { active, sessions } = this.state
      const index = sessions.indexOf(active)
    ...
    ipcRenderer.on('Move Shell Backward', () => {
      const { active, sessions } = this.state
      const index = sessions.indexOf(active)

      if (index > 0) {
        sessions.splice(index - 1, 0, sessions.splice(index, 1)[0])
        this.setState({ sessions })
      }
    })

    ipcRenderer.on('Move Shell Forward', () => {
      const { active, sessions } = this.state
      const index = sessions.indexOf(active)
      ...
      <div className={styles.container} onClick={() => this.appClicked()}>
        ...
        {sessions.map((ses, i) => (
          <div
            ...
            key={ses.pty.pid}>
            <Term pty={ses.pty} active={ses === active} autoFocus={!findVisible} zoom={zoom} />
          </div>
        ))}
      </div>
    )
