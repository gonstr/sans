...
class App extends React.Component {
  ...
  poll = async shell => {
    ...
    if (shell.cwd !== pidInfo.cwd) shell.branch = await proc.gitInfo(pidInfo.cwd)
  ...
  render() {
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.footer}>{activeShell && activeShell.branch}</div>
      </div>
    )
  }
}
...
const styles = {
  ...
  term: {
    position: 'absolute',
    top: '50px',
    bottom: '40px', // Not symetric but looks best since term height from bottom is inconsistent
    left: '15px',
