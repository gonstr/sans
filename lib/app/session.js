...
const config = require(path.join(os.homedir(), `.sans/config.json`))
...
function createPty(cwd) {
  ...
  return spawn(config.shell || '/bin/bash', ['--login'], {
    name: 'xterm-color',
    cols: 100,
    rows: 40,
    cwd,
...
export default class Session extends EventEmitter {
  constructor(cwd = os.homedir()) {
    ...
    if (config.shell === '/bin/zsh') {
      this.pty.write(`source ${path.join(os.homedir(), '.sans/scripts/zsh-init.sh')} && clear\n`)
    } else {
      this.pty.write(
        `source ${path.join(
          os.homedir(),
          '.sans/scripts/bash-init.sh'
