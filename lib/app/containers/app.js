...
import { newTab, hideFind, showTab, findNext, setFindValue } from '../actions'
import { findActiveSession } from '../utils'
...
class App extends Component {
  ...
  tabHeaderClicked = tab => {
    if (!this.moved) this.props.dispatch(showTab(tab.id))
  }

  findValueChanged = ({ target }) => this.props.dispatch(setFindValue(target.value))
  findBlurred = () => this.props.dispatch(hideFind())

  render() {
    const { tabs, activeTab, fullscreen, findValue, findVisible } = this.props

    function buildSessionNodes(tab, autoFocus) {
      const node = tab.node
      if (node.type === 'ses') {
        return <Term pty={node.data.pty} active={tab === activeTab} autoFocus={autoFocus} />
      ...
      <div className={styles.container} onClick={() => this.appClicked()}>
        ...
        {tabs.map((tab, i) => (
          <div className={cx(styles.term, { [appstyles.hidden]: tab !== activeTab })} key={tab.id}>
            {buildSessionNodes(tab, !findVisible)}
          </div>
        ))}
      </div>
    )
